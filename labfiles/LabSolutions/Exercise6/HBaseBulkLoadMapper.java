...
public class HBaseBulkLoadMapper extends Mapper<LongWritable, Text, ImmutableBytesWritable, Put> {
	public void map(LongWritable key, Text value, Context context) {
		...
		try {
			...
			put.add(Bytes.toBytes(configuration.get("COLUMN_FAMILY_1")), Bytes.toBytes("first_name"), Bytes.toBytes(inputdata[1]));
			put.add(Bytes.toBytes(configuration.get("COLUMN_FAMILY_1")), Bytes.toBytes("last_name"), Bytes.toBytes(inputdata[2]));
			// Add email and phone_nymber to COLUMN_FAMILY_2 with their respective inputs given above.
			// Use the add function of the Put variable and make sure all the variables are in Bytes!
			put.add(Bytes.toBytes(configuration.get("COLUMN_FAMILY_2")), Bytes.toBytes("email"), Bytes.toBytes(inputdata[3]));
			put.add(Bytes.toBytes(configuration.get("COLUMN_FAMILY_2")), Bytes.toBytes("phone_number"), Bytes.toBytes(inputdata[4]));
			// Finalize it by using the write function on the Context variable using the hbase table name and the Put variable
			context.write(new ImmutableBytesWritable(Bytes.toBytes(configuration.get("hbase.table.name"))), put);
		}
